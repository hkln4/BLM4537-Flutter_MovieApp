...
class WatchlistBloc extends Bloc<WatchlistEvent, WatchlistState> {
  ...
  Future<void> _getWatchListItems(
    ...
    final result = await _getWatchListItemsUseCase.call(event.userId);
  ...
  Future<void> _addWatchListItem(
    ...
    final result = await _addWatchListItemUseCase.call(
      AddWatchlistItemParams(media: event.media, userId: event.userId),
    );
    ...
    result.fold(
      ...
      (r) => emit(const WatchlistState(actionStatus: BookmarkStatus.added, isBookmarked: true)),
    );
  }
  ...
  Future<void> _removeWatchListItem(
    ...
    final result = await _removeWatchListItemUseCase.call(
      RemoveWatchlistItemParams(movieId: event.movieId, userId: event.userId),
    );
    ...
    result.fold(
      ...
      (r) => emit(const WatchlistState(actionStatus: BookmarkStatus.removed, isBookmarked: false)),
    );
  }
  ...
  FutureOr<void> _checkBookmark(
    ...
    final result = await _isBookmarkedUseCase.call(
      IsBookmarkedParams(tmdbId: event.tmdbId, userId: event.userId),
    );
    ...
    result.fold(
      ...
      (r) => emit(WatchlistState(
        actionStatus: BookmarkStatus.exists,
        isBookmarked: r,
      )),
    );
  }
